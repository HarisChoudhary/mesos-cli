...
def import_modules(package_paths, module_type):
    ...
    for package_path in package_paths:
        ...
        try:
            ...
            (file, filename, data) = imp.find_module(package_name, [package_dir])
...
def completions(comp_words, current_word, argv):
    ...
    if len(argv) == 1 and current_word and argv[0] not in comp_words:
        return comp_words

    if len(argv) == 1 and current_word and argv[0] in comp_words:
        return comp_words

    if len(argv) == 1 and not current_word and argv[0] not in comp_words:
        return []
...
def format_subcommands_help(cmd):
    ...
    return (arguments, short_help, long_help)
