...
class Container(PluginBase):
    ...
    def __all(self,command):
        agents = self.__get_agents(argv)
        if len(agents) == 0:
            print("No agents running in cluster!")
        else:
            for agent in agents:
                print ("Agent:  "+agent[0])
                self.remoteCall(argv, agent[1],command)

    #Helper function that allows us to get all agents from the Master. Return list in the form tuple (Agent ID, Agent IP)
    def __get_agents(self, argv):
        try:
            httpResponse = urllib.request.urlopen("http://"+config.MASTER_IP+"/slaves").read().decode('utf-8')
        except Exception as e:
            print(e)
            return []

        agentJSON = json.loads(httpResponse);
        agentInfo = []

        for agent in agentJSON["slaves"]:
            agentInfo.append((agent["id"],agent["pid"].split("@")[1]))
